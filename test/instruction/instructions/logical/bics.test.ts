...
describe('test canEncodeInstruction function for BICS', () => {
  it('should be able to encode instruction with correct register information', () => {
    expect(bicsInstruction.canEncodeInstruction('BICS',['R1', 'R1', 'R7'])).toBeTruthy()
  })

  it('should not be able to encode instruction with wrong register information', () => {
    expect(bicsInstruction.canEncodeInstruction('BICS',['R1', 'R7', 'R7'])).toBeFalsy()
  })

  it('should not be able to encode instruction with wrong register information', () => {
    expect(bicsInstruction.canEncodeInstruction('BICS',['R1', 'R7'])).toBeFalsy()
  })

  it('should not be able to encode instruction with wrong instruction name', () => {
    expect(bicsInstruction.canEncodeInstruction('BIC',['R1', 'R1','R7'])).toBeFalsy()
  })
...
describe('test encode instruction function for BICS', () => {
  ...
  it('should throw an error for 2 params', () => {
    expect(() =>
        bicsInstruction.encodeInstruction(['R1', 'R7'], {})
    ).toThrow()
  })
...
describe('test executeInstruction function for BICS', () => {
  it('should return correct value from register for BICS R1, R2,', () => {
    ...
    expect(registers.readRegister(Register.R1).value).toEqual(0b00000000100010000000000000000100)
