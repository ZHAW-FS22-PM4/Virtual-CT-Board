...
beforeEach(function () {
  registers.reset()
  registers.writeRegister(Register.R1, Word.fromUnsignedInteger(0b00000001000010000000010010101101))
  registers.writeRegister(Register.R2, Word.fromUnsignedInteger(0b00000000010000000000011110101011))
  registers.writeRegister(Register.R3, Word.fromUnsignedInteger(0b00000000000000000000011110101011))
...
describe('test canEncodeInstruction function for EORS', () => {
  it('should encode be able to instruction with correct register information', () => {
    expect(eorsInstruction.canEncodeInstruction('EORS',['R1', 'R1', 'R7'])).toBeTruthy()
  })

  it('should not be able to encode instruction with wrong register information', () => {
    expect(eorsInstruction.canEncodeInstruction('EORS',['R1', 'R7', 'R7'])).toBeFalsy()
  })

  it('should not be able to encode instruction with wrong register information', () => {
    expect(eorsInstruction.canEncodeInstruction('EORS',['R1', 'R7'])).toBeFalsy()
  })

  it('should not be able to encode instruction with wrong instruction name', () => {
    expect(eorsInstruction.canEncodeInstruction('EOR',['R1', 'R1','R7'])).toBeFalsy()
  })
...
describe('test encode instruction function for EORS', () => {
  ...
  it('should throw an error for 2 params', () => {
    expect(() =>
      eorsInstruction.encodeInstruction(['R1', 'R7'], {})
    ).toThrow()
  })
...
describe('test executeInstruction function for EORS', () => {
  it('should return correct value from register for EORS R1, R2,', () => {
    ...
    expect(registers.readRegister(Register.R1).value).toEqual(0b00000001010010000000001100000110)
